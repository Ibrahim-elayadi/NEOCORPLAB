// Translation dictionary
const translations = {
    en: {
        nav_models: "Models",
        nav_718: "718 Cayman & Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "About",
        nav_heritage: "Heritage",
        nav_technology: "Technology",
        nav_sustainability: "Sustainability",
        nav_careers: "Careers",
        nav_services: "Services",
        nav_tequipment: "Porsche Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "Financing",
        nav_warranty: "Warranty",
        nav_contact: "Contact",
        hero_title: "Performance Redefined",
        hero_subtitle: "Experience the pinnacle of automotive engineering and luxury",
        hero_button: "EXPLORE MODELS",
        models_title: "Our Iconic Models",
        model_911: "Iconic sports car with rear engine and two doors",
        model_718: "Agile mid-engine sports car",
        model_taycan: "Electric sports sedan with cutting-edge technology",
        model_panamera: "Luxury sports sedan with grand touring capabilities",
        model_macan: "Compact performance SUV",
        model_cayenne: "Versatile performance SUV",
        model_learnmore: "Learn More →"
    },
    fr: {
        nav_models: "Modèles",
        nav_718: "718 Cayman & Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "À propos",
        nav_heritage: "Héritage",
        nav_technology: "Technologie",
        nav_sustainability: "Durabilité",
        nav_careers: "Carrières",
        nav_services: "Services",
        nav_tequipment: "Porsche Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "Financement",
        nav_warranty: "Garantie",
        nav_contact: "Contact",
        hero_title: "Performance Redéfinie",
        hero_subtitle: "Découvrez le summum de l'ingénierie automobile et du luxe",
        hero_button: "EXPLORER LES MODÈLES",
        models_title: "Nos Modèles Emblématiques",
        model_911: "Voiture de sport emblématique avec moteur arrière et deux portes",
        model_718: "Voiture de sport agile avec moteur central",
        model_taycan: "Berline de sport électrique avec technologie de pointe",
        model_panamera: "Berline de sport luxe avec capacités de grand tourisme",
        model_macan: "SUV de sport compact",
        model_cayenne: "SUV de sport polyvalent",
        model_learnmore: "En savoir plus →"
    },
    de: {
        nav_models: "Modelle",
        nav_718: "718 Cayman & Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "Über uns",
        nav_heritage: "Erbe",
        nav_technology: "Technologie",
        nav_sustainability: "Nachhaltigkeit",
        nav_careers: "Karrieren",
        nav_services: "Dienstleistungen",
        nav_tequipment: "Porsche Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "Finanzierung",
        nav_warranty: "Garantie",
        nav_contact: "Kontakt",
        hero_title: "Leistung neu definiert",
        hero_subtitle: "Erleben Sie das Nonplusultra der Automobilingenieurkunst und des Luxus",
        hero_button: "MODELLE ERKUNDEN",
        models_title: "Unsere ikonischen Modelle",
        model_911: "Ikonisches Sportauto mit Heckmotor und zwei Türen",
        model_718: "Wendiges Mittelmotor-Sportauto",
        model_taycan: "Elektrische Sportlimousine mit modernster Technologie",
        model_panamera: "Luxus-Sportlimousine mit Grand-Touring-Fähigkeiten",
        model_macan: "Kompaktes Performance-SUV",
        model_cayenne: "Vielseitiges Performance-SUV",
        model_learnmore: "Mehr erfahren →"
    },
    es: {
        nav_models: "Modelos",
        nav_718: "718 Cayman & Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "Acerca de",
        nav_heritage: "Herencia",
        nav_technology: "Tecnología",
        nav_sustainability: "Sostenibilidad",
        nav_careers: "Carreras",
        nav_services: "Servicios",
        nav_tequipment: "Porsche Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "Financiamiento",
        nav_warranty: "Garantía",
        nav_contact: "Contacto",
        hero_title: "Rendimiento Redefinido",
        hero_subtitle: "Experimenta lo máximo en ingeniería automotriz y lujo",
        hero_button: "EXPLORAR MODELOS",
        models_title: "Nuestros Modelos Icónicos",
        model_911: "Auto deportivo icónico con motor trasero y dos puertas",
        model_718: "Auto deportivo ágil con motor central",
        model_taycan: "Sedán deportivo eléctrico con tecnología de punta",
        model_panamera: "Sedán deportivo de lujo con capacidades de gran turismo",
        model_macan: "SUV de rendimiento compacto",
        model_cayenne: "SUV de rendimiento versátil",
        model_learnmore: "Saber más →"
    },
    it: {
        nav_models: "Modelli",
        nav_718: "718 Cayman & Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "Chi Siamo",
        nav_heritage: "Eredità",
        nav_technology: "Tecnologia",
        nav_sustainability: "Sostenibilità",
        nav_careers: "Carriere",
        nav_services: "Servizi",
        nav_tequipment: "Porsche Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "Finanziamento",
        nav_warranty: "Garanzia",
        nav_contact: "Contatti",
        hero_title: "Prestazioni Ridefinite",
        hero_subtitle: "Sperimenta l'apice dell'ingegneria automobilistica e del lusso",
        hero_button: "ESPLORA MODELLI",
        models_title: "I Nostri Modelli Iconici",
        model_911: "Auto sportiva iconica con motore posteriore e due porte",
        model_718: "Auto sportiva agile con motore centrale",
        model_taycan: "Berlina sportiva elettrica con tecnologia all'avanguardia",
        model_panamera: "Berlina sportiva di lusso con capacità di gran turismo",
        model_macan: "SUV prestazionale compatto",
        model_cayenne: "SUV prestazionale versatile",
        model_learnmore: "Scopri di più →"
    },
    ar: {
        nav_models: "الموديلات",
        nav_718: "718 كايمان وبوكستر",
        nav_taycan: "تايكان",
        nav_panamera: "بناميرا",
        nav_macan: "ماكان",
        nav_cayenne: "كايين",
        nav_about: "حول",
        nav_heritage: "التراث",
        nav_technology: "التكنولوجيا",
        nav_sustainability: "الاستدامة",
        nav_careers: "الوظائف",
        nav_services: "الخدمات",
        nav_tequipment: "بورشه تيكويبمنت",
        nav_exclusive: "منفاكتور إكسكلوسيف",
        nav_financing: "التمويل",
        nav_warranty: "الضمان",
        nav_contact: "اتصل",
        hero_title: "الأداء معاد التعريف",
        hero_subtitle: "اختبر قمة الهندسة السيارات والفخامة",
        hero_button: "استكشف الموديلات",
        models_title: "موديلاتنا الشهيرة",
        model_911: "سيارة رياضية أيقونية بمحرك خلفي وبابين",
        model_718: "سيارة رياضية رشيقة بمحرك وسط",
        model_taycan: "سيارة سيدان رياضية كهربائية بتكنولوجيا متقدمة",
        model_panamera: "سيارة سيدان رياضية فاخرة مع قدرات الجولة الكبرى",
        model_macan: "سيارة رياضية متعددة الأغراض مدمجة",
        model_cayenne: "سيارة رياضية متعددة الأغراض متعددة الاستخدامات",
        model_learnmore: "اعرف المزيد →"
    },
    zh: {
        nav_models: "车型",
        nav_718: "718 卡宴和 Boxster",
        nav_taycan: "Taycan",
        nav_panamera: "Panamera",
        nav_macan: "Macan",
        nav_cayenne: "Cayenne",
        nav_about: "关于",
        nav_heritage: "遗产",
        nav_technology: "技术",
        nav_sustainability: "可持续性",
        nav_careers: "职业生涯",
        nav_services: "服务",
        nav_tequipment: "保时捷 Tequipment",
        nav_exclusive: "Exclusive Manufaktur",
        nav_financing: "融资",
        nav_warranty: "保修",
        nav_contact: "联系",
        hero_title: "重新定义的性能",
        hero_subtitle: "体验汽车工程和奢华的顶峰",
        hero_button: "探索车型",
        models_title: "我们的标志性车型",
        model_911: "采用后置发动机和两门设计的标志性跑车",
        model_718: "采用中置发动机的灵活跑车",
        model_taycan: "具有尖端技术的电动跑车轿车",
        model_panamera: "具有长途旅行能力的豪华跑车轿车",
        model_macan: "紧凑型性能SUV",
        model_cayenne: "多功能性能SUV",
        model_learnmore: "了解更多 →"
    },
    ja: {
        nav_models: "モデル",
        nav_718: "718 ケイマン&ボクスター",
        nav_taycan: "タイカン",
        nav_panamera: "パナメーラ",
        nav_macan: "マカン",
        nav_cayenne: "カイエン",
        nav_about: "について",
        nav_heritage: "ヘリテージ",
        nav_technology: "テクノロジー",
        nav_sustainability: "サステナビリティ",
        nav_careers: "キャリア",
        nav_services: "サービス",
        nav_tequipment: "ポルシェ テキップメント",
        nav_exclusive: "エクスクルーシブ マニュファクトゥール",
        nav_financing: "ファイナンシング",
        nav_warranty: "ワランティ",
        nav_contact: "お問い合わせ",
        hero_title: "再定義されたパフォーマンス",
        hero_subtitle: "自動車エンジニアリングと高級の頂点を体験してください",
        hero_button: "モデルを探索する",
        models_title: "私たちのアイコニックモデル",
        model_911: "リアエンジン2ドア仕様のアイコニックスポーツカー",
        model_718: "ミッドシップエンジン搭載の俊敏なスポーツカー",
        model_taycan: "最先端テクノロジーを搭載した電動スポーツセダン",
        model_panamera: "グランドツーリング能力を備えた高級スポーツセダン",
        model_macan: "コンパクトパフォーマンスSUV",
        model_cayenne: "多目的パフォーマンスSUV",
        model_learnmore: "詳しく →"
    }
};

// Google Translate initialization
function googleTranslateElementInit() {
    new google.translate.TranslateElement({
        pageLanguage: 'en',
        includedLanguages: 'en,fr,de,es,it,ar,zh-CN,ja',
        layout: google.translate.TranslateElement.InlineLayout.SIMPLE,
        autoDisplay: false
    }, 'google_translate_element');
}

// Language code mapping for Google Translate
const langCodeMap = {
    en: 'en',
    fr: 'fr',
    de: 'de',
    es: 'es',
    it: 'it',
    ar: 'ar',
    zh: 'zh-CN',
    ja: 'ja'
};

// Function to trigger Google Translate
function triggerGoogleTranslate(langCode) {
    const googleTranslateCode = langCodeMap[langCode];
    if (googleTranslateCode) {
        const element = document.querySelector('.goog-te-combo');
        if (element) {
            element.value = googleTranslateCode;
            element.dispatchEvent(new Event('change'));
        }
    }
}

// Function to change language
function changeLanguage(lang) {
    const elements = document.querySelectorAll('[data-i18n]');
    elements.forEach(element => {
        const key = element.getAttribute('data-i18n');
        if (translations[lang] && translations[lang][key]) {
            element.textContent = translations[lang][key];
        }
    });
    document.documentElement.lang = lang;
    
    // Trigger Google Translate for the rest of the page
    setTimeout(() => {
        triggerGoogleTranslate(lang);
    }, 100);
}

// Initialize event listeners
document.addEventListener('DOMContentLoaded', () => {
    const langOptions = document.querySelectorAll('.lang-option');
    const menuToggle = document.querySelector('.menu-toggle');
    const sideMenu = document.querySelector('.side-menu');
    const sideMenuOverlay = document.querySelector('.side-menu-overlay');
    const mainContent = document.querySelector('.main-content');
    const nav = document.querySelector('nav');
    const closeBtn = document.querySelector('.menu-close-btn');

    // Close menu function
    const closeMenuHandler = () => {
        sideMenu.classList.remove('active');
        sideMenuOverlay.classList.remove('active');
        mainContent.classList.remove('blur');
        nav.classList.remove('blur');
        closeBtn.classList.remove('active');
    };

    // Menu toggle click handler
    if (menuToggle) {
        menuToggle.addEventListener('click', (e) => {
            e.stopPropagation();
            sideMenu.classList.toggle('active');
            sideMenuOverlay.classList.toggle('active');
            mainContent.classList.toggle('blur');
            nav.classList.toggle('blur');
            closeBtn.classList.toggle('active');
        });
    }

    // Close menu when clicking on overlay
    if (sideMenuOverlay) {
        sideMenuOverlay.addEventListener('click', () => {
            closeMenuHandler();
        });
    }

    // Close button click handler
    if (closeBtn) {
        closeBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            closeMenuHandler();
        });
    }

    // Close menu when clicking on a menu item
    const menuItems = document.querySelectorAll('.side-menu a');
    menuItems.forEach(item => {
        item.addEventListener('click', () => {
            closeMenuHandler();
        });
    });

    // Language selection handler
    langOptions.forEach(option => {
        option.addEventListener('click', (e) => {
            e.preventDefault();
            const lang = option.getAttribute('data-lang');
            
            // Update button text (if langToggle exists)
            const langToggle = document.getElementById('langToggle');
            if (langToggle) {
                langToggle.textContent = lang.toUpperCase() + ' 🌐';
            }
            
            // Update active state
            langOptions.forEach(opt => opt.classList.remove('active'));
            option.classList.add('active');
            
            // Change page language
            changeLanguage(lang);
            
            // Store language preference
            localStorage.setItem('selectedLanguage', lang);
        });
    });
});
        // Hide loading screen after 4 seconds (4000ms total)
        window.addEventListener('load', () => {
            setTimeout(() => {
                document.getElementById('loadingScreen').classList.add('hidden');
            }, 4000);
        });

